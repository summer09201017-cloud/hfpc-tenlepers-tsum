/* 麻風十人・回來感謝 —— tsum 連鏈版(物理堆疊+劃線連同款)
 * 反向化鑰匙:連線不是「消滅」,是「結伴回來感謝耶穌」——十個得潔淨,只有一個回來;
 * 願我們作那回來的一個。經文:路17:11-19(和合本,已 cuv 查驗)。
 * 零相依、可離線、手機直向友善。榮耀歸神。
 */
(function(){
'use strict';
...
// ---------- tsum 圖鑑:十個得潔淨的人(路17:14「他們去的時候就潔淨了」;顏色拉開+穿戴記號) ----------
var TYPES = [
  {id:'p0', kind:'person', name:'紅頭巾', c1:'#cf5a4a', c2:'#a83c30', headband:true},
  {id:'p1', kind:'person', name:'藍披肩', c1:'#5a87c9', c2:'#3d64a0', wrap:true},
  {id:'p2', kind:'person', name:'綠腰帶', c1:'#6aa557', c2:'#4b823c', belt:true},
  {id:'p3', kind:'person', name:'黃斗篷', c1:'#d9a83c', c2:'#b28327', cloak:true},
  {id:'p4', kind:'person', name:'紫圓帽', c1:'#9a6bb8', c2:'#77488f', cap:true},
  {id:'p5', kind:'person', name:'白鬍爺爺', c1:'#b8b29e', c2:'#948e7a', beard:true},
  {id:'p6', kind:'person', name:'粉辮子', c1:'#d98a9c', c2:'#b06679', braids:true}
];
...
var modeKey = 'kid';
try{ modeKey = localStorage.getItem('tlp-mode') || 'kid'; }catch(e){}
...
var BURST = { icon:'🙌', name:'大聲歸榮耀', banner:'🙌 大聲歸榮耀與神!附近的都回來了!' };
...
var TROUBLE = { id:'tr', name:'岔路的狐狸', c1:'#8a5a36', c2:'#5c3a22', fc:'#a87a50', ec:'#f4e8d8', trouble:true };
...
function maxLevel(){ try{ return Math.max(1, parseInt(localStorage.getItem('tlp-lvl-'+modeKey))||1); }catch(e){ return 1; } }
function loadStars(){ try{ stars = JSON.parse(localStorage.getItem('tlp-stars-'+modeKey)||'{}')||{}; }catch(e){ stars = {}; } }
function saveStars(lv, st){
  if ((stars[lv]|0) >= st) return;
  stars[lv] = st;
  try{ localStorage.setItem('tlp-stars-'+modeKey, JSON.stringify(stars)); }catch(e){}
}
...
function sprintEnd(){
  ...
  speak('win');
  if (!doneSent){ doneSent = true; if (window.__ping) window.__ping('tenlepers-tsum-done', Math.round((Date.now()-startTime)/1000)); }
}
...
var CAP = 46;
var muted = false;
try{ muted = localStorage.getItem('tlp-mute') === '1'; }catch(e){}
...
function activeTypes(){
  ...
  if (M.types === 4) return [TYPES[0], TYPES[1], TYPES[3], TYPES[4]];
  if (M.types === 5) return [TYPES[0], TYPES[1], TYPES[3], TYPES[4], TYPES[6]];
  if (M.types === 6) return [TYPES[0], TYPES[1], TYPES[3], TYPES[4], TYPES[6], TYPES[2]];
  return TYPES;
}
...
function bgmSeed(lv){
  ...
  var x = 2166136261, s = 'tenlepers-tsum|' + (lv || 1);
...
function collect(list){
  ...
  banner = { text: n>=5 ? ('好長的一隊!帶回 '+people+' 份感謝!') : ('帶回 '+people+' 份感謝'), t:1.4 };
  if (troubles > 0){ banner = { text:'😈 ' + TROUBLE.name + '混進來了!−' + (troubles*TROUBLE_PENALTY), t:1.6 }; blip(160, 0.2, 'square', 0.1); }
  else if (goldN > 0){ banner = { text:'✨ 撒馬利亞人回來了!多獻上 ' + (goldN*mult) + ' 份感謝', t:1.6 }; }
  ...
  if (chainCount >= nextBlessAt && blessT<=0){
    ...
    banner = { text:'✨ 起來,走吧!你的信救了你了——感恩加倍!', t:2.4 };
  ...
  if (!sprint && fed >= curTarget && !won){
    ...
    if (!doneSent){ doneSent = true;
      if (window.__ping) window.__ping('tenlepers-tsum-done', Math.round((Date.now()-startTime)/1000)); }
  }
}
...
function rescue(){
  ...
  banner = { text:"✨ 主的憐憫——同伴聚在一起了!", t:2.0 };
...
function drawTsum(t, xx, yy, rr){
  ...
  if (!ty.cap && !ty.headband){ // 頭髮(耳前無髮)
    ctx.fillStyle = 'rgba(60,42,24,.85)';
    ctx.beginPath(); ctx.arc(x, y-r*0.3, r*0.6, Math.PI*1.1, Math.PI*1.9); ctx.fill();
  }
  if (ty.headband){ ctx.strokeStyle='#7a2a20'; ctx.lineWidth=r*0.14;
    ctx.beginPath(); ctx.arc(x, y-r*0.28, r*0.62, Math.PI*1.15, Math.PI*1.85); ctx.stroke(); }
  if (ty.wrap){ ctx.strokeStyle='rgba(255,255,255,.75)'; ctx.lineWidth=r*0.16; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(x-r*0.55, y+r*0.42); ctx.lineTo(x+r*0.45, y-r*0.48); ctx.stroke(); }
  if (ty.belt){ ctx.strokeStyle='rgba(40,60,30,.7)'; ctx.lineWidth=r*0.15; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(x-r*0.66, y+r*0.5); ctx.lineTo(x+r*0.66, y+r*0.5); ctx.stroke(); }
  if (ty.cloak){ ctx.strokeStyle='rgba(120,80,30,.8)'; ctx.lineWidth=r*0.08; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(x-r*0.45, y+r*0.3); ctx.lineTo(x, y+r*0.48); ctx.lineTo(x+r*0.45, y+r*0.3); ctx.stroke();
    ctx.fillStyle='#8a5a1e'; ctx.beginPath(); ctx.arc(x, y+r*0.48, r*0.11, 0, 7); ctx.fill(); }
  if (ty.cap){ ctx.fillStyle='#5a3d78';
    ctx.beginPath(); ctx.arc(x, y-r*0.5, r*0.46, Math.PI, 0); ctx.fill(); }
  if (ty.beard){ ctx.fillStyle='rgba(255,255,255,.9)';
    ctx.beginPath(); ctx.arc(x, y+r*0.34, r*0.32, 0, Math.PI); ctx.fill(); }
  if (ty.braids){ ctx.strokeStyle='#6a4a2a'; ctx.lineWidth=r*0.12; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(x-r*0.6, y-r*0.18); ctx.lineTo(x-r*0.66, y+r*0.36); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x+r*0.6, y-r*0.18); ctx.lineTo(x+r*0.66, y+r*0.36); ctx.stroke(); }
...
function drawHUD(){
  ctx.fillStyle = '#2e5c4a';
  ctx.fillRect(0,0,W,CROWD_TOP);
  ctx.fillStyle = '#fff'; ctx.font = 'bold 26px "Microsoft JhengHei",sans-serif'; ctx.textAlign='center';
  if (sprint){ ctx.fillText('⏱ ' + Math.ceil(sprintT) + ' 秒・感謝 ' + Math.round(shownFed) + ' 份', W/2, 40); }
  else { ctx.fillText('已獻上感謝 ' + Math.round(shownFed) + ' / ' + curTarget + ' 份', W/2, 40); }
  // 返回大廳
...
function hudTap(p){
  if (p.y < CROWD_TOP){
    ...
    if (p.x > W-56){ muted = !muted; try{ localStorage.setItem('tlp-mute', muted?'1':'0'); }catch(e){} return true; }
  }
  return false;
}
...
function drawMenu(t){
  ...
  ctx.fillText('麻風十人', W/2, 190);
  ctx.font = 'bold 34px "Microsoft JhengHei",sans-serif';
  ctx.fillText('回 來 感 謝', W/2, 245);
  // 示意 tsum
  var demo = [TYPES[0], TYPES[1], TYPES[3], TYPES[4], TYPES[5]];
  for (var i=0;i<5;i++) drawTsum({t:demo[i], hi:0}, 90+i*90, 330 + Math.sin(t*2+i)*8, 34);
  ctx.fillStyle = '#fff'; ctx.font = '22px "Microsoft JhengHei",sans-serif';
  ctx.fillText('「內中有一個見自己已經好了,', W/2, 420);
  ctx.fillText('就回來大聲歸榮耀與 神。」(路17:15)', W/2, 452);
  ctx.font = '24px "Microsoft JhengHei",sans-serif'; ctx.fillStyle = '#cfe9d5';
  ctx.fillText('劃線連起同款的得潔淨的人,', W/2, 510);
  ctx.fillText('結伴回來,向耶穌獻上感謝!', W/2, 544);
  menuBtns = [];
  ...
  for (i=0;i<3;i++){
    ...
    ctx.fillText('同款連 ' + MODES[keys[i]].minChain + ' 顆・感謝 ' + MODES[keys[i]].target + ' 份', W/2, y+60);
...
function menuTap(p){
  for (var i=0;i<menuBtns.length;i++){
    ...
    if (p.x>b.x && p.x<b.x+b.w && p.y>b.y && p.y<b.y+b.h){
      ...
      try{ localStorage.setItem('tlp-mode', modeKey); }catch(e){}
...
function startGame(forceLv){
  ...
  scene = 'play'; playing = true; startTime = Date.now();
    if (sprint){ banner = { text: '⏱ 限時衝刺!' + Math.round(SPRINT_SECS[modeKey]||80) + ' 秒內帶回越多感謝越好!', t: 3 }; }
  else banner = { text: '劃線連起 ' + M.minChain + ' 顆同款!', t: 3 };   // 07-22:各檔連鏈門檻不同(青少年=4),開場講清楚
  ac(); bgmStart(); speak('intro');
  if (window.__ping) window.__ping('tenlepers-tsum-start');
}
var winBtns = [];
function drawWin(t){
  drawScene(t);
  ...
  ctx.fillText(sprint ? ('⏱ 時間到!共獻上 ' + fed + ' 份感謝') : '🎉 回來感謝主!', W/2, 170);
  ctx.font = 'bold 42px sans-serif'; ctx.fillStyle = '#ffd54a';
  ctx.fillText(('★★★').slice(0, lastStars) + ('☆☆☆').slice(0, 3-lastStars), W/2, 218);
  // 十個得潔淨的人:一個回來俯伏感謝(發光),九個走遠了(灰)
  var bx0 = W/2 - 4.5*46, by = 262;
  for (i=0;i<10;i++){
    var bx = bx0 + i*46, back = i > 0;
    var appear = Math.min(1, Math.max(0, (t*4 - i*0.3)));
    if (appear <= 0.05) continue;
    if (!back){
      ctx.strokeStyle = 'rgba(255,225,120,'+(0.5+0.3*Math.sin(t*4)).toFixed(2)+')'; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.arc(bx, by-2, 26, 0, 7); ctx.stroke();
    }
    ctx.fillStyle = back ? 'rgba(220,220,220,.4)' : '#d98a3c';
    if (back){
      ctx.beginPath(); ctx.arc(bx, by-10, 8, 0, 7); ctx.fill();
      ctx.fillRect(bx-7, by-3, 14, 16);
    } else {
      ctx.beginPath(); ctx.arc(bx-8, by+2, 8, 0, 7); ctx.fill();
      ctx.fillRect(bx-12, by+8, 24, 7);
    }
  }
  ctx.fillStyle = '#fff'; ctx.font = '23px "Microsoft JhengHei",sans-serif';
  var L = ['「內中有一個見自己已經好了,','就回來大聲歸榮耀與 神,','又俯伏在耶穌腳前感謝他;','這人是撒馬利亞人。」','(路加福音 17:15-16)'];
  for (i=0;i<L.length;i++) ctx.fillText(L[i], W/2, 350 + i*40);
  ctx.fillStyle = '#cfe9d5'; ctx.font = '22px "Microsoft JhengHei",sans-serif';
  ctx.fillText('十個得潔淨,只有一個回來。', W/2, 590);
  ctx.fillText('願我們作那回來的一個——凡事謝恩。', W/2, 624);
  winBtns = [];
...
function winTap(p){
  for (var i=0;i<winBtns.length;i++){
    var b = winBtns[i];
    if (p.x>b.x && p.x<b.x+b.w && p.y>b.y && p.y<b.y+b.h){
      if (b.act==='next'){ var nx = level+1; try{ localStorage.setItem('tlp-lvl-'+modeKey, ''+Math.max(maxLevel(), nx)); }catch(e){} startGame(nx); return; }
